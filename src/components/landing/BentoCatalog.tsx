...
export default function BentoCatalog({
  ...
  // ── Fetch products by category from API (FIX: not local filter) ──
  const fetchByCategory = useCallback(async (slug: string) => {
  ...
  // ── When category changes, fetch from API ──
  useEffect(() => {
  ...
  }, [activeCategory, initialProducts, totalProducts, fetchByCategory]);
  ...
  return (
    <section id="katalog" className="py-20 sm:py-28 bg-white overflow-x-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* ── Empty State ── */}
        {!isLoadingCategory && visibleCount === 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
