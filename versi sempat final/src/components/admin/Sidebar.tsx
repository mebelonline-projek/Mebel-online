...
export default function Sidebar({ logoUrl = "" }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`hidden lg:flex fixed top-0 left-0 z-40 h-full bg-white border-r border-gray-200 flex flex-col transition-all duration-300 ${
          collapsed ? "w-16" : "w-64"
        }`}
      >
        {/* Brand */}
        <div className="flex items-center justify-between h-16 px-4 border-b border-gray-100">
          {!collapsed && (
            <Link href="/admin/dashboard" className="flex items-center gap-2">
              ...
              {logoUrl ? (
                <div className="relative w-8 h-8 shrink-0 rounded-lg overflow-hidden ring-1 ring-brand-maroon/10">
                  <Image
                    src={logoUrl}
                    alt="Logo"
                    fill
